...
function getAllowedWindowForShift(shiftSetting, now = new Date()) {
  const [openHour, openMinute] = shiftSetting.startTime.split(":").map(Number);

  // Window absensi dihitung dari jam buka (startTime), bukan dari endTime.
  const openedAt = new Date(now);
  openedAt.setHours(openHour, openMinute, 0, 0);
...
export async function getTodaysScheduledAttendanceForUser(userId, now = new Date()) {
  ...
  return {
    ...
    existingAttendance,
    attendanceLabel: existingAttendance ? getAttendanceStatusLabel(existingAttendance.status) : null,
    isBeforeWindow,
    isAfterWindow,
