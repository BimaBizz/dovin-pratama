...
export function buildAttendancePhotoAbsolutePath(photoPath) {
  return path.join(/*turbopackIgnore: true*/ process.cwd(), photoPath.replace(/^\/+/, ""));
}
...
export async function cleanupExpiredAttendancePhotos() {
  ...
  // Delete all expired photo files in parallel (much faster than sequential)
  await Promise.allSettled(
    expiredRecords
      .filter((record) => record.photoPath)
      .map(async (record) => {
        ...
        try {
          ...
          if (error?.code !== "ENOENT") {
            console.error(`[cleanup] Failed to delete ${record.photoPath}:`, error?.message);
          }
        }
      })
  );

  // Single batch DB update instead of N individual updates
