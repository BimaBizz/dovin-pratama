...
function resolveRolePriority(rolePriorityMap, role) {
  const directRole = String(role || "").trim().toUpperCase();
  if (directRole && Number.isFinite(Number(rolePriorityMap?.[directRole]))) {
    return Number(rolePriorityMap[directRole]);
  }

  const normalizedTarget = normalizeRoleKey(role);
  if (!normalizedTarget) {
  ...
  for (const [configuredRole, priority] of Object.entries(rolePriorityMap || {})) {
    if (normalizeRoleKey(configuredRole) === normalizedTarget && Number.isFinite(Number(priority))) {
      matchedPriority = Number(priority);
    }
  }
...
export async function getPermissionEvaluator(role) {
  ...
  const rolePriority = resolveRolePriority(rolePriorityMap, role);
